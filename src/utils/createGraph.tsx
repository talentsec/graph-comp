...
const colorList = ['#a5abb6', '#c990c0', '#f79767', '#57c7e3', '#f16667', '#d9c8ae']
...
export class VizSimulation {
  ...
  create() {
    ...
    // const ringElement = svg.append('g')
    //   .attr('class', 'rings')
    //   .selectAll('circle')
    //   .data(nodes)
    //   .enter().append('circle')
    //   .attr('r', d => 2.8 * (d.height + 2))
    //   .attr('fill', '#4c8eda')
    //   .call(drag)
    ...
    //   .data(nodes)
    //   .enter().append('circle')
    //   .attr('r', d => 2.6 * (d.height + 2))
    //   .attr('fill', d => colorList[d.height])
    //   .call(drag)
    ...
    this.simulation.nodes(nodes as SimulationNodeDatum[]).on('tick', () => {
      if (this.vizType === 'tree') {
        // FIXME:
        ringElement.attr('cx', (d: any) => d.x).attr('cy', d => {
